...
public class ProcessingUnit extends Thread implements ProcessingUnitInterface,
		ReportObservable, SOHObserver {

	// Defining Inputs from Sensors
	private Float kmLeftInBattery;
	private Float currentDistanceTraveled; // This is the distance that Car
											// traveled in current session
	...
	ProcessingUnit(BatteryReport batteryReportCharge, SOHSystem sohSystem)
	...
	public Integer getChargingCyclesLeft() {
		...
		return ((Integer) BMS.getDataInCollection(BMS.CHARGING_CYCLES) - (Integer) BMS
				.getDataInCollection(BMS.CHARGING_CYCLES_USED));
	}

	// Calculating this distance after every 1 sec
	public void setDistanceTravelledByCar() throws ValueOutOfBoundException {
		float previousDistance = (Float) BMS
				.getDataInCollection(BMS.DISTANCE_TRAVELLED);
		...
		BMS.storeDataInCollection(BMS.DISTANCE_TRAVELLED,
				(currentDistanceTraveled + previousDistance));

		nextNearestPumpDistance = nextNearestPumpDistance
	...
	public Float getDistanceLeftInBattery() throws ValueOutOfBoundException {
		...
		Float chargeInBattery = (Float) BMS
				.getDataInCollection(BMS.BATTERY_CHARGE_AMOUNT); // Dummy
																	// Variable,
																	// need to
																	// insert
																	// the
	...
	public Float getTimeLeftInBattery() throws ValueOutOfBoundException {
		Float distanceLeft = getDistanceLeftInBattery(); // Dummy Variable, need
															// to insert the
															// function which
															// will get charge
															// from Charge group
	...
	public void storeChargingBatteryLevel() {
		// TODO Auto-generated method stub
		Integer batteryLevel = (Integer) BMS
				.getDataInCollection(BMS.BATTERY_LEVEL) + 10; // Dummy Variable,
																// need to
																// insert the
																// function
																// which will
	...
	public void showBatteryLevel() {
		...
		System.out.println("Battery Level : "
				+ BMS.getDataInCollection(BMS.BATTERY_LEVEL));
	}
	...
	public void updateBatteryChargeLevelLeft(Float _distanceTravelled)
			throws ValueOutOfBoundException {
		// TODO Auto-generated method stub
		Float chargeLeft = ((Float) BMS
				.getDataInCollection(BMS.BATTERY_CHARGE_AMOUNT))
				- (consumptionRate * _distanceTravelled);
	...
	public Integer showAlerts(Alert alert) {
		...
		if (alert.getType() > 0) {
			...
			if ((alert.toString()).equals((Alert.ALERT_BATTERYLOW).toString())) {
				System.out.println("\n\nBattery Low");
				returnVal = 1;
			} else if ((alert.toString()).equals((Alert.ALERT_OVERCHARGE)
					.toString())) {
				System.out.println("\n\nBattery Overcharge");
				returnVal = 2;
			} else if ((alert.toString()).equals((Alert.ALERT_HIGHTEMP)
					.toString())) {
				System.out.println("\n\nBattery has High Temperature");
				returnVal = 3;
			} else if ((alert.toString()).equals((Alert.ALERT_DAMAGE)
					.toString())) {
	...
	@Override
	public void update() {
		if ((this.batteryReportCharge.getAlert().toString())
				.equals((soc.Alert.OVER_DISCHARGE).toString())) {
			alert = Alert.ALERT_BATTERYLOW;
		} else if ((this.batteryReportCharge.getAlert().toString())
				.equals((soc.Alert.OVERCHARGE).toString())) {
			alert = Alert.ALERT_OVERCHARGE;
		} else if ((this.batteryReportCharge.getAlert().toString())
				.equals((soc.Alert.OVERHEATING).toString())) {
	...
	public void execute() {
		try {
				...
				if (BMS.getBMSStatus().equals(BMSState.ONMOVE.toString())) {
					...
					System.out
							.println("Charge Amount : "
									+ BMS.getDataInCollection(BMS.BATTERY_CHARGE_AMOUNT));
					...
					System.out.format(
							"Time Left for next charge : %.2f hours \n",
							getTimeLeftInBattery());
					System.out.println("Charging Cycles left : "
							+ getChargingCyclesLeft());

					System.out.format("Distance to next Pump : %.2f Km \n",
							Math.abs(gps.getNextNearestPumpDistance()));

					System.out
							.format("Battery Temperature : %.1f degree celcius \n",
									BMS.getDataInCollection(BMS.CURRENT_BATTERY_TEMPERATURE));
					int SOH = (Integer) BMS
							.getDataInCollection(BMS.BATTERY_HEALTH);
					if (SOH <= 10) {
						System.out
								.format("Battery Health : %d %% (Low health! Please change battery)\n",
										SOH);
					} else {
						System.out.format("Battery Health : %d %% \n", SOH);
					}
					System.out
							.format("Remaining Useful Life : %d days \n",
									(Integer) BMS
											.getDataInCollection(BMS.BATTERY_LIFE));

					/*
					 * if (alert.getType() > 0) { showAlerts(alert); }
					 */

					System.out.println("\n------ GUI Output End -------\n\n");

					printSystemLog();
				} else if (BMS.getBMSStatus().equals(
						BMSState.CHARGING.toString())) {
					System.out
							.println("------ Presenting GUI Output -------\n");
			...
			System.err.println("Exception occured 1 : "
	...
	public void printSystemLog() {
		System.out
				.println("--------------- System Log Information --------------");
		...
		System.out.println("\n\nLoad/Consumption Rate:"
				+ BMS.getDataInCollection(BMS.CAR_LOAD));
		// Log from Health Group
		System.out.println("Battery Capacity: "
				+ BMS.getDataInCollection(BMS.PRESENTCAPACITY) + " MHA");
		System.out.println("Remaining Useful Life : "
				+ BMS.getDataInCollection(BMS.BATTERY_LIFE) + " days");
		System.out.println("Battery Healthy : "
				+ BMS.getDataInCollection(BMS.BATTERY_HEALTH) + " %");
		System.out
				.println("--------------- System Log Information Ends --------------");
	...
	public void run() {
		...
		} while ((Float) BMS.getDataInCollection(BMS.BATTERY_CHARGE_AMOUNT) > 0
				&& (Integer) BMS.getDataInCollection(BMS.BATTERY_LEVEL) < 100
				&& !BMS.getBMSStatus().toString()
						.equals(BMSState.DAMAGED.toString()));
